...
class PrepareProductForExport(models.TransientModel):
    ...
    def export_direct_in_woo(self, product_templates):
        ...
        for variant in variants:
            ...
            if product_template.categ_id:
                self.create_categ_in_woo(product_template.categ_id, woo_instance.id)  # create category
    ...
    def import_products_from_csv(self):
        ...
        for record in file_data:
            ...
            if not woo_template:
                if categ_obj:
                    self.create_categ_in_woo(categ_obj, instance_id.id)  # create category
    ...
    def create_categ_in_woo(self, categ_id, instance, ctg_list=[]):
        ...
        if categ_id:
            ...
            for categ_id in list(OrderedDict.fromkeys(reversed(ctg_list))):
                woo_product_parent_categ = categ_id.parent_id and woo_product_categ.search(
                    [('name', '=', categ_id.parent_id.name), ('woo_instance_id', '=', instance)],
                    limit=1) or False
                if woo_product_parent_categ:
                    woo_product_category = woo_product_categ.search([('name', '=', categ_id.name), (
                        'parent_id', '=', woo_product_parent_categ.id), ('woo_instance_id', '=',
                                                                         instance)], limit=1)
                else:
                    woo_product_category = woo_product_categ.search(
                        [('name', '=', categ_id.name), ('woo_instance_id', '=', instance)],
                        limit=1)
                if not woo_product_category:
                    if not categ_id.parent_id:
                        ...
                        woo_product_categ.create(
                            {
                                'name': categ_id.name, 'woo_instance_id': instance,
                                'parent_id': parent_id.id
                            })
                elif not woo_product_category.parent_id and categ_id.parent_id:
                    parent_id = woo_product_categ.search([('name', '=', categ_id.parent_id.name), (
    ...
    def update_category_info(self, categ_obj, instance_id):
        ...
        self._cr.execute(
            "select id from woo_product_categ_ept where LOWER(name) = '%s' and "
            "woo_instance_id = %s limit 1" % (ctg, instance_id))
        ...
        if not woo_categ_id:
            woo_categ_id = woo_product_categ.create(
                {'name': categ_obj.name, 'woo_instance_id': instance_id})
        else:
